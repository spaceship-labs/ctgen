...
describe('format', function(){
  ...
  describe('xlsx2csv', function(){
    ...
    it('should format csv, used alias for head instead replace', function(done) {
      var expectCsvHead = '"namefieldhead1","namefieldhead2","namefieldhead3"\n' +
              '"field1","field2","field3"\n'+
              ',"field2.1","field3.1"\n' +
              ',,"field3.2 with comm\'"\n' +
              ',,"field4.2 a"\n';
      format.xls2csv_stream('testfile.xls', {headCsv: '"namefieldhead1","namefieldhead2","namefieldhead3"', HeadCsvAsAlias: true}, function(err, csv){
        stubs.fs.createOutputStream.called.should.be.equal(false);
        csv.should.be.equal(expectCsvHead);
        done();
      });
