...
file.format = function(root, fileNames, files) {
  ...
  files.sort().forEach(function(f) {
    var list = Path.dirname(f).split(Path.sep),
        indexRoot = list.indexOf(root.replace('/', '')),
        name = indexRoot == -1 || indexRoot + 1  == list.length ? list[list.length-1] : list[indexRoot + 1], // list.length -> indexRoot 
        index = objsName.indexOf(name);

    console.log('indexRoot',indexRoot,list,name);
