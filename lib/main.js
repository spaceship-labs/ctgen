...
runScript.mongoScript = function(db, script){
  if(main.verbose)
    console.log('mongo ' + db + ' ' + scriptPath + script + ' ' + main.authString);
  return shell.exec('mongo ' + db + ' ' + scriptPath + script + ' ' + main.authString);
};
...
runScript.importScv = function(files, db, collection, done){
  ...
  */
  async.eachSeries(files, function(file, next){
    if(main.verbose)
      console.log('mongoimport -d '+ db +' -c ' + collection +' --file ' + '"'+ file  +'" --type csv --headerline '+main.authString);
    var res = shell.exec('mongoimport -d '+ db +' -c ' + collection +' --file ' + '"'+ file  +'" --type csv --headerline '+main.authString);
    if(main.verbose)
