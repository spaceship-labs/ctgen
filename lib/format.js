...
format.xls2csv_stream = function(file, opts, done) {
  ...
  if (opts.requiredColumns) {
    opts.aliasHeadCsv = opts.aliasHeadCsv || {};
    opts.requiredColumns.forEach(function(e) {
      opts.aliasHeadCsv[e] = e;
    });
  }
  ...
  reader.on('open', function(book) {
        ...
        if (!header && opts.headCsv && opts.headCsv.length) {
          ...
          row.forEach(function(cell) {
            ...
            if (val && val.replace) {
              val = val.replace(replaceEs1, '\'');
              val = val.replace(replaceEs2, "");
              if (!header && opts.headCsvLowerCase) {
                val = val.toLowerCase();
              }
              if (!header && opts.aliasHeadCsv) {
                if (opts.aliasHeadCsv[val]) {
                  val = opts.aliasHeadCsv[val];
                  aliasSets[val] = true;
                }
              }
              val = '"' + val + '"';
    ...
    if (opts.aliasHeadCsv) {
      warningMissingColumns(opts, aliasSets, file);
    }
    if (dest && !dest.raw)
      dest.end();
...
function warningMissingColumns(opts, aliasSets, file) {
  var alias = Object.keys(opts.aliasHeadCsv);
  var aliasSetsLength = Object.keys(aliasSets).length;
  if (aliasSetsLength < alias.length) {
    alias.forEach(function(e) {
      var set = opts.aliasHeadCsv[e];
      if (!aliasSets[set]) {
        var warn = 'WARN!, Missing column ' + e + ' ';
        if (e != set) {
          warn += '-> '+ set + ' ';
        }
        warn += 'in '+ file;
  ...
  async.concatSeries(sourceDatas, function(src, next) {
    format.getFiles(src.docs, function(err, files) {
      ...
      async.concatSeries(files, function(file, nextFile) {
        ...
        if (src.headCsvLowerCase) {
          opts.headCsvLowerCase = src.headCsvLowerCase;
        }

        opts.requiredColumns = src.requiredColumns;
        opts.aliasHeadCsv = src.aliasHeadCsv;
...
format.xls2csv_rUpdate = function(sourceDatas, done) {
  // necesito las rutas completas al csv
  // separado por carpeta padre name
  var rootPath = process.cwd();
  async.concatSeries(sourceDatas, function(src, next) {
    console.log('DOCS',src.docs);
    if( src.docs.length == 0 ) return next(null,src);
    format.getFiles(src.docs, function(err, files) {
      if(format.verbose)
